...
if ( ! class_exists( 'YITH_Tab_Manager_Table' ) ) {
	...
	class YITH_Tab_Manager_Table extends WP_List_Table {
		...
		public function prepare_items() {
			...
			$this->is_trash = isset( $_GET['status'] ) && 'trash' == $_GET['status'];
		...
		public function column_cb( $item ) {
			return sprintf(
				'<input type="checkbox" name="ywctm_ids[]" value="%s" />', $item->ID
			);
		}
				...
				case 'title' :
					...
					if ( $delete ) {
						...
						$actions['delete'] = '<a href="' . esc_url( get_delete_post_link( $item, '',
								true ) ) . '" class="submitdelete">' . __( 'Delete permanently', 'yith-woocommerce-tab-manager' ) . '</a>';
					} else {
					...
					echo sprintf( '<strong><a class="tips" target="_blank" href="%s" data-tip="%s">#%d %s </a></strong> %s', $action_edit_url, __( 'Edit', 'yith-woocommerce-tab-manager' ), $item->ID, $post_title, $this->row_actions( $actions ) );
					break;
				case 'is_show' :
