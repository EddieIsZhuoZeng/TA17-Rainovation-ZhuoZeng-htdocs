...
class Grid extends \WPGMZA\MarkerListing
{
	public function __construct($map_id)
	{
		\WPGMZA\MarkerListing::__construct($map_id);
		$this->element->addClass('wpgmza-marker-grid');
	}
	...
	public function getAjaxResponse($request)
	{
		...
		$document->loadPHPFile($this->getItemHTMLPath() . 'grid-item.html.php');
		...
		foreach($response->data as $marker)
		{
			...
			if($wpgmza->settings->useLegacyHTML)
			{
				...
				$img_wrap = $item->querySelector('.wpgmza-grid-image-wrap');
				if(!empty($marker->pic)) {
				...
				// Title
				$title = $item->querySelector('.wpgmza_marker_title');
				...
				if (empty($marker->link) && !$directions) {
					if ($el = $item->querySelector('.wpgmza-grid-footer')) {
						$el->remove();
					}
				}
