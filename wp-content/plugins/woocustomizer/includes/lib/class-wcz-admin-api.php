	...
	public function display_field( $data = array(), $post = null, $echo = true ) {
		...
		switch ( $field['type'] ) {
			...
			case 'promo':
				...
				$html .= '<img class="wcz_promo_image ' . sanitize_html_class( $option_name ) . '" src="' . esc_url( WCD_PLUGIN_URL . '/assets/images/' . $option_name . '.jpg' ) . '" />' . "\n";
				break;

			case 'doclink': ?>
				<div class="wcz-doclink">
					<a href="<?php echo esc_url( $field['placeholder'] ); ?>" target="_blank"><?php esc_html_e( 'View Documentation', 'woocustomizer' ); ?></a>
				</div>
				<?php
				break;
